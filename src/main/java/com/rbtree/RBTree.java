...
//		return search2(root, key); //使用递归的方法，本质一样的
	...
	//删除红色节点，无子节点
	private void removeRed(RBNode<T> node){
	...
	/**
	 * 1.兄弟节点是黑色，兄弟节点的子节点是红色，这里面又分为几种情况见代码
	 * @param node
	 */
  private void brotherBlackNephewRed(RBNode<T> node,RBNode<T> parent){
	  if (parent.left.right != null && parent.left.right.color==RED){
		  //左右
		  RBNode<T> newParent = parent.left.right;
		  //左右旋
	  ...
	  }else if(parent.left.left != null && parent.left.left.color == RED){
		  RBNode<T> newParent = parent.left;
		  leftRotate(parent);
		  //变色
		  newParent.color = parent.color;
		  newParent.left.color = BLACK;
	...
	/**
	 *  2.兄弟节点是黑色，无子节点，父节点是红色
	 * @param node
	 */
  private void brotherBlackFatherRed(RBNode<T> node,RBNode<T>parent){


  }

	/**
	 * 3.兄弟是红色，兄弟节点的两个子节点是黑色
	 */
  private void brotherRedNephewBlack(RBNode<T> brother,RBNode<T> parent){
  	RBNode<T> newParent= null;
	...
	/**
	 * 4.兄弟节点是黑色的，兄弟节点无子节点 brotherBlack
	 *
	 */
	private  void  brotherBlack(RBNode<T> brother,RBNode<T> parent){
		//变色
		parent.color = BLACK;
		brother.color=RED;
		remove(parent,false);
	...
	/**删除黑色节点，无子节点
	 * 分几种情况
	 * 1.兄弟节点是黑色，兄弟节点的子节点是红色，这里面又分为几种情况见代码   brotherBlackNephewRed
	 * 2.兄弟节点是黑色，无子节点，父节点是红色 brotherBlackFatherRed
	 * 3.兄弟是红色，兄弟节点的两个子节点是黑色  brotherRedNephewBlack
	 * 4.兄弟节点是黑色的，兄弟节点无子节点 brotherBlack
	 *
	 * @param node
	 */
	private void  removeBlack(RBNode<T> node){
		/**
		 * 1.兄弟节点是黑色，兄弟节点的子节点是红色
		 */
		RBNode<T> parent = node.parent;
		if (node == parent.left ){
			if (parent.right.color == BLACK){
				if (parent.right.left ==null && parent.right.right ==null ){
					brotherBlack(parent.right,parent);//4.兄弟节点是黑色的，兄弟节点无子节点 brotherBlack
				}else {
					brotherBlackNephewRed(node,parent);//1.兄弟节点是黑色，兄弟节点的子节点是红色
				}

			}else {
				brotherRedNephewBlack(parent.right,parent);
			}
			...
			if (parent.left.color == BLACK){
				...
				brotherRedNephewBlack(parent.left,parent);
			}
	...
	// 删除一个子节点的情况
	private void removeOneChild(RBNode<T> node){
		RBNode<T> child = node.left== null?node.right:node.left;
	...
	//删除有两个子节点的节点
	private void removeBothChild(RBNode<T> node ){
		//找后继节点
		...
		//交互节点
		swapTreeElem(node,next);
		remove(next ,true);


	}
	//交换值
	private void  swapTreeElem(RBNode<T> node, RBNode<T>next){
	...
	private void remove(RBNode<T> node,boolean delete) {
		if(node.left == null && node.right ==null ){
			...
			//子节点为空
			/**
			 * 只有一个子节点的情况，比较简单，被删节点必须为黑色，子节点必为红色，其他情况均不存在
			 */
			removeOneChild(node);

		}else {
			//both child node are not null，左右子节点为非空
